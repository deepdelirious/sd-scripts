...
def train(args):
    ...
    _, teacher = flux_utils.load_flow_model(
        args.teacher, weight_dtype, args.teacher_device, args.disable_mmap_load_safetensors
    )
    ...
    for epoch in range(num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(*training_models):
                ...
                with accelerator.autocast():
                    ...
                    with torch.no_grad():
                        teacher_pred = teacher(
                            img=packed_noisy_model_input.to(args.teacher_device),
                            img_ids=img_ids.to(args.teacher_device),
                            txt=t5_out.to(args.teacher_device),
                            txt_ids=txt_ids.to(args.teacher_device),
                            y=l_pooled.to(args.teacher_device),
                            timesteps=timesteps.to(args.teacher_device) / 1000,
                            guidance=guidance_vec.to(args.teacher_device),
                            txt_attention_mask=t5_attn_mask.to(args.teacher_device) if t5_attn_mask else t5_attn_mask,
                        )
                ...
                # calculate loss
                supervised_loss = train_util.conditional_loss(
                    model_pred.float(), target.float(), reduction="none", loss_type=args.loss_type, huber_c=None
                )
                distillation_loss = torch.nn.functional.mse_loss(model_pred.float(), teacher_pred.to(model_pred.device).float(), reduction="none")
                loss = (1 - args.distillation_alpha) * supervised_loss + args.distillation_alpha * distillation_loss
